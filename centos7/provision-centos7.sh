...
if [[ $? -eq 1 ]] ;then
   ...
   echo "192.168.33.10   client client.box vagrant-client" >> /etc/hosts
   echo "192.168.33.15   server server.box vagrant-server" >> /etc/hosts
...
# Install RPMs on client and server VMs
packages=( syslinux cifs-utils genisoimage net-tools samba samba-client tftp
           bind-utils rear mtools attr libusal sshfs )
